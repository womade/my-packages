...
function check_update()
		...
		updatelogs = luci.sys.exec("curl -skfL https://op.ssss.fun/firmware/" ..model.. "/updatelogs.txt")
...
function to_check()
	...
	if model == "x86_64" then
		...
		if fs.access("/sys/firmware/efi") then
			download_url = "https://op.ssss.fun/firmware/" ..model.. "/" ..dateyr.. "-openwrt-x86-64-generic-squashfs-combined-efi.img.gz"
		else
			download_url = "https://op.ssss.fun/firmware/" ..model.. "/" ..dateyr.. "-openwrt-x86-64-generic-squashfs-combined.img.gz"
			md5 = ""
		end
    elseif model:match(".*R2S.*") then
		model = "nanopi-r2s"
		check_update()
			download_url = "https://op.ssss.fun/firmware/" ..model.. "/" ..dateyr.. "-openwrt-rockchip-armv8-nanopi-r2s-squashfs-sysupgrade.img.gz"
    elseif model:match(".*R4S.*") then
		model = "nanopi-r4s"
		check_update()
			download_url = "https://op.ssss.fun/firmware/" ..model.. "/" ..dateyr.. "-openwrt-rockchip-armv8-nanopi-r4s-squashfs-sysupgrade.img.gz"
    elseif model:match(".*R2C.*") then
		model = "nanopi-r2c"
		check_update()
			download_url = "https://op.ssss.fun/firmware/" ..model.. "/" ..dateyr.. "-openwrt-rockchip-armv8-nanopi-r2c-squashfs-sysupgrade.img.gz"
    elseif model:match(".*Pi 4 Model B.*") then
		model = "Rpi-4B"
		check_update()
		download_url = "https://op.ssss.fun/firmware/" ..model.. "/" ..dateyr.. "-openwrt-bcm27xx-bcm2711-rpi-4-squashfs-sysupgrade.img.gz"
    else
